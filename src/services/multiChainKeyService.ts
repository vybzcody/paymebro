...
type SupportedChainId = CctpNetworkId;
...
export class MultiChainKeyService {
  ...
  async getAccountForChain(chainId: CctpNetworkId): Promise<{ address: string; signer?: any }> {
    ...
    // Handle Solana chains (both mainnet and devnet)
    if (normalizedChainId === CctpNetworkId.SOLANA || normalizedChainId === 'solana' ||
        normalizedChainId === CctpNetworkId.SOLANA_DEVNET || normalizedChainId === 'solana-devnet') {
    ...
    // Handle EVM chains (both mainnet and testnet)
    if (normalizedChainId === CctpNetworkId.ETHEREUM || normalizedChainId === 1 ||
        normalizedChainId === CctpNetworkId.ETH_SEPOLIA || normalizedChainId === 11155111 ||
        ...
        normalizedChainId === CctpNetworkId.BASE_SEPOLIA || normalizedChainId === 84532 ||
        normalizedChainId === CctpNetworkId.POLYGON || normalizedChainId === 137 ||
        normalizedChainId === CctpNetworkId.POLYGON_AMOY || normalizedChainId === 80002 ||
        normalizedChainId === CctpNetworkId.AVALANCHE || normalizedChainId === 43114 ||
        normalizedChainId === CctpNetworkId.AVAX_FUJI || normalizedChainId === 43113) {
  ...
  async getAllAccounts(): Promise<Record<CctpNetworkId, string>> {
    ...
    try {
      console.log('Getting all accounts, IS_TESTNET:', IS_TESTNET);
      ...
      if (IS_TESTNET) {
        ...
        console.log('Set Solana Devnet account:', solanaAccount.address);
        ...
        accounts[CctpNetworkId.POLYGON_AMOY as CctpNetworkId] = ethAccount.address;
        accounts[CctpNetworkId.AVAX_FUJI as CctpNetworkId] = ethAccount.address;
        console.log('Set EVM testnet accounts:', ethAccount.address);
      } else {
        // Use mainnet chains
        accounts[CctpNetworkId.SOLANA as CctpNetworkId] = solanaAccount.address;
        
        const ethAccount = await this.getEthereumAccount();
        accounts[CctpNetworkId.ETHEREUM as CctpNetworkId] = ethAccount.address;
        accounts[CctpNetworkId.ARBITRUM as CctpNetworkId] = ethAccount.address;
        accounts[CctpNetworkId.BASE as CctpNetworkId] = ethAccount.address;
        accounts[CctpNetworkId.POLYGON as CctpNetworkId] = ethAccount.address;
        accounts[CctpNetworkId.AVALANCHE as CctpNetworkId] = ethAccount.address;
      }
