...
import { Link, DollarSign, Search, Copy, Eye, Trash2, Zap, QrCode, Download } from "lucide-react";
...
import { generatePaymentQR, downloadQRCode } from "@/services/qrService";
import { useState } from "react";
import { toast } from "sonner";

const PaymentLinks = () => {
  const { paymentLinks, loading, deletePaymentLink, refetch } = usePaymentLinks();
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [selectedLink, setSelectedLink] = useState(null);
  const [deleteModal, setDeleteModal] = useState({ open: false, linkId: '', linkTitle: '' });
  const [qrModal, setQrModal] = useState({ open: false, qrCode: '', title: '' });
  ...
  const handleGenerateQR = async (link: any) => {
    try {
      const qrCode = await generatePaymentQR(link.payment_url || link.url);
      setQrModal({ open: true, qrCode, title: link.title });
    } catch (error) {
  ...
  const handleDownloadQR = () => {
    if (qrModal.qrCode) {
      downloadQRCode(qrModal.qrCode, `${qrModal.title}-qr.png`);
      ...
      <div className="flex justify-between items-center">
        ...
        <Dialog>
          ...
          <DialogContent className="max-w-3xl">
            ...
            <MultiChainPaymentForm onSuccess={() => {
              refetch();
                    ...
                    <div className="text-right space-y-1">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => copyToClipboard(link.payment_url || link.url)}
                        >
                          <Copy className="h-4 w-4" />
                        </Button>
                        <Button
          ...
          <DialogContent className="max-w-md">
            ...
            <div className="flex flex-col items-center space-y-4">
              <div className="text-center">
                <h3 className="font-medium">{qrModal.title}</h3>
                <p className="text-sm text-gray-500">Scan to pay</p>
              </div>
              {qrModal.qrCode && (
                <div className="bg-white p-4 rounded-lg border">
                  <img src={qrModal.qrCode} alt="Payment QR Code" className="w-64 h-64" />
                </div>
              )}
