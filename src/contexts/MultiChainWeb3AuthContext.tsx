...
export const MultiChainWeb3AuthProvider: React.FC<MultiChainWeb3AuthProviderProps> = ({ children }) => {
  ...
  const refreshBalances = useCallback(async () => {
    ...
    try {
      ...
      for (const [chainId, address] of Object.entries(allAccounts)) {
        if (address && updatedWallets[chainId as CctpNetworkId]) {
          const balance = await BalanceService.getBalanceForChain(address, chainId as CctpNetworkId);
          updatedWallets[chainId as CctpNetworkId].balance = balance;
        }
      }
  ...
  const initializeAllChainWallets = async (provider: IProvider, keyServiceInstance: MultiChainKeyService) => {
    try {
      ...
      console.log('Received accounts from keyService:', accounts);
      
      const newWallets = { ...wallets };
      console.log('Initial wallets state:', newWallets);

      // Initialize each chain wallet with proper type conversion
      for (const [chainIdStr, address] of Object.entries(accounts)) {
        if (address && chainIdStr && chainIdStr !== 'undefined') {
          console.log(`Processing chain: ${chainIdStr}, address: ${address}`);
          ...
          try {
            ...
            const isTestnetSolana = chainIdStr === 'solana-devnet';
            const isMainnetSolana = chainIdStr === 'solana';
          
            newWallets[chainId] = {
              provider: (isTestnetSolana || isMainnetSolana) ? provider : null,
              address,
              balance: { native: 0, usdc: 0 },
              signer
            };
            
            console.log(`Initialized ${chainId} wallet:`, {
              address: address.slice(0, 8) + '...' + address.slice(-8),
              hasProvider: !!(isTestnetSolana || isMainnetSolana),
              hasSigner: !!signer
            });
          } catch (error) {
            console.error(`Failed to initialize wallet for ${chainIdStr}:`, error);
          }
        }
      }

      console.log('Final wallets object before setState:', newWallets);
