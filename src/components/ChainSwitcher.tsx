...
export const ChainSwitcher: React.FC = () => {
  ...
  const getChainType = (chainId: CctpNetworkId): 'solana' | 'evm' => {
    return chainId === CctpNetworkId.SOLANA ? 'solana' : 'evm';
  };
      ...
      <Select value={activeChain.toString()} onValueChange={handleChainSwitch}>
        <SelectTrigger className="w-auto min-w-[160px]">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium">{activeNetwork?.name || 'Select Network'}</span>
        ...
        <SelectContent>
          {supportedNetworks.map((network) => {
            ...
            const chainType = getChainType(network.id);
