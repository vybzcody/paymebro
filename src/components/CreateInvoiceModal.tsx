...
export const CreateInvoiceModal = ({ isOpen, onClose }: CreateInvoiceModalProps) => {
  ...
  const [invoiceData, setInvoiceData] = useState({
    ...
    // CCTP settings
    cctpEnabled: true,
    preferredChain: CctpNetworkId.SOLANA,
    autoConvert: true,
  });
  ...
  const handleCreateInvoice = async () => {
    ...
    try {
      // Get merchant wallet address from keyService
      let merchantWallet = '';
      if (keyService) {
        const solanaAccount = await keyService.getSolanaAccount();
        merchantWallet = solanaAccount.address;
      }
            ...
            <div className="border rounded-lg p-4 space-y-4 bg-blue-50/50">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <Label className="text-base font-medium">Cross-Chain Payment (CCTP)</Label>
                </div>
                <Switch
                  checked={invoiceData.cctpEnabled}
                  onCheckedChange={(checked) => setInvoiceData(prev => ({ ...prev, cctpEnabled: checked }))}
                />
              </div>
              
              {invoiceData.cctpEnabled && (
                <>
                  <div className="space-y-2">
                    <Label>Preferred Receive Chain</Label>
                    <Select 
                      value={invoiceData.preferredChain} 
                      onValueChange={(value) => setInvoiceData(prev => ({ ...prev, preferredChain: value as CctpNetworkId }))}
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="Select chain to receive USDC" />
                      </SelectTrigger>
                      <SelectContent>
                        {networkAdapters.map((network) => (
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      checked={invoiceData.autoConvert}
                      onCheckedChange={(checked) => setInvoiceData(prev => ({ ...prev, autoConvert: checked }))}
                    />
                  </div>
                  ...
                  <div className="flex items-start gap-2 p-3 bg-blue-100 rounded-md">
                    ...
                    <div className="text-sm text-blue-800">
                      <p className="font-medium">Customer can pay from any supported chain:</p>
                      <p className="text-xs mt-1">Ethereum, Arbitrum, Base, Polygon, Avalanche, Solana</p>
                    </div>
                  </div>
                </>
              )}
