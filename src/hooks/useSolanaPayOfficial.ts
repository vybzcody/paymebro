...
export const useSolanaPayOfficial = () => {
  ...
  /**
   * Create Transaction Request (interactive payment)
   * Uses backend endpoint for complex payment flows
   */
  const createTransactionRequest = useCallback(async (paymentData: PaymentData) => {
    if (!publicKey) {
      toast({
        title: 'Wallet Not Connected',
        description: 'Please connect your wallet first',
    ...
    try {
      ...
      const transactionRequestUrl = solanaPayService.createAfriPayTransactionRequest({
        reference: backendRequest.paymentRequest.reference,
        amount: paymentData.amount,
        currency: paymentData.currency,
      ...
      setPaymentState({
        status: 'pending',
        paymentUrl: transactionRequestUrl,
        qrCodeUrl: backendRequest.paymentRequest.qrCodeUrl,
        reference: new PublicKey(backendRequest.paymentRequest.reference),
        feeBreakdown: backendRequest.feeBreakdown,
        expiresAt: backendRequest.paymentRequest.expiresAt,
      ...
      toast({
        ...
        description: `Interactive payment flow ready for ${paymentData.amount.toFixed(4)} ${paymentData.currency}`,
      });
      ...
      const errorMessage = error.message || 'Failed to create transaction request';
      setPaymentState({ status: 'failed', error: errorMessage });
  ...
  }, [publicKey, user, isBackendHealthy, toast, solanaPayService]);
