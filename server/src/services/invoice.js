...
export class InvoiceService {
  ...
  async createInvoice({
    ...
    try {
      ...
      const merchantName = merchant?.business_name || merchant?.name || 'AfriPay Merchant';
      
      // Generate unique reference
      const reference = this.generateReference();
      
      // Create Solana Pay URL instead of generic payment URL
      const baseUrl = process.env.FRONTEND_URL || 'http://localhost:5173';
      const paymentUrl = `${baseUrl}/pay/${reference}`;
      ...
      const { data: invoice, error: insertError } = await supabase
        ...
        .insert({
          ...
          merchant_name: merchantName, // Add merchant name
          amount,
          currency,
